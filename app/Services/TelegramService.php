...
class TelegramService
{
    ...
    public static function sendMessage($chatId, $message, $keyboard = '', $type = '', $mediaFiles = [], 
        $entities = null, $topicId = null, $replyToMessageId = null)
    {
        $results = [];
        ...
        if (!empty($mediaFiles) && count($mediaFiles) > 0) {
            ...
            if (count($mediaFiles) == 1) {
                ...
                if (!empty($replyToMessageId)) {
                    $json['json']['reply_to_message_id'] = $replyToMessageId;
                }
    ...
    private static function log($chatId, $form_params, $error = NULL, $messageId = NULL, $type = NULL, $response = NULL) {
        // For channels/groups, use null for tid (only positive numbers are user IDs)
        $tid = (is_numeric($chatId) && $chatId > 0) ? $chatId : null;
    ...
    public static function forwardMessage($fromChatId, $toChatId, $messageId, $topicId = null)
    {
        ...
        return self::send($toChatId, $json, 'forwardMessage');
    }
    ...
    {
        // Skip if chatId is not a positive number (channels/groups have @username or negative IDs)
